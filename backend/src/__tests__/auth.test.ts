...
describe('employee', () => {
  ...
  describe('given that an employee wants to access their account', () => {
    it('using default password "Demco@2023" should return an arror', async () => {
      const { statusCode, body } = await request(app)
        .post('/auth/login')
        .set('Accept', 'application/json')
        .send({ email: 'amarachigoodness74@gmail.com', password: 'Demco@2023' });
  ...
  describe('given that an employee forgot their password', () => {
    it('should collect email address and send a reset password link', async () => {
      const { statusCode, body } = await request(app)
        ...
        .set('Accept', 'application/json')
        .send({ email: 'amarachigoodness74@gmail.com' });
